...
curl "${CURL_UPL_OPTS[@]}" --request PUT --upload-file "$PKG" \
  "$ARTIFACTORY_URL/$REPO_KEY/pool/main/${PKGBN:0:1}/${PKGBN/%_*/}/$PKGBN$DISTS;deb.component=main;deb.architecture=amd64" \
  || exit
...
curl "${CURL_UPL_OPTS[@]}" --request POST \
  "$ARTIFACTORY_URL/api/deb/reindex/$REPO_KEY?async=0" || exit
