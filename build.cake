    ...
    .Does(() =>
{
    ...
    DockerRunWithoutResult(new DockerContainerRunSettings {
        ...
        Volume = new string[] { 
            ...
            $"{Environment.CurrentDirectory}/linux-package-feeds:/linux-package-feeds",
            $"{Environment.CurrentDirectory}/artifacts:/artifacts"
        }
    }, "octopusdeploy/package-linux-docker:latest", "bash /BuildAssets/create-linux-packages.sh");
    ...
    CopyFileToDirectory($"{assetDir}/repos/test-octo-feed-packages.sh", $"{artifactsDir}/linuxpackages");
    CopyFileToDirectory($"{assetDir}/repos/test-octo-feed-package.sh", $"{artifactsDir}/linuxpackages");
    CopyFileToDirectory($"{linuxPackageFeedsDir}/test-linux-packages.sh", $"{artifactsDir}/linuxpackages");
